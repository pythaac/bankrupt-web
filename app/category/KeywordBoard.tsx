...
import { apiServerUrl, IApiResult, ICategoryBundle } from '../common/Constants'
...
function getKewordColumns(
  categoryBundleApiResult: IApiResult<ICategoryBundle>,
) {
  ...
  async function onClickAddKewordButton(data: any) {
    if (!!category) {
      const responsePost = await fetch(apiServerUrl + '/v1/category/resource', {
        method: 'POST',
        body: new Blob([JSON.stringify({ ...data, categoryId: category.id })], {
  ...
  return [
    <Text key="keyword_board_text">Keywords</Text>,
    <InputDialog<KeywordBody>
      key="keyword_board_dialog"
      title="새로운 키워드 추가"
...
function addTrashbin(
  categoryResoruceId: number,
  onClickDeleteCategoryResourceButton: Function,
) {
  return (
    <AlertDialog
      onClick={() => onClickDeleteCategoryResourceButton(categoryResoruceId)}
    >
...
function getKewordItems(categoryBundleApiResult: IApiResult<ICategoryBundle>) {
  const keywordTable: Array<Array<ReactNode>> = []

  async function onClickDeleteCategoryResourceButton(
    categoryResourceId: number,
  ) {
    const responseDelete = await fetch(
      apiServerUrl + '/v1/category/resource/' + categoryResourceId,
      {
        method: 'DELETE',
      },
    )
  ...
  if (!!keywords) {
    keywords.map((keyword) => {
      keywordTable.push([
        <Text key={`keyword_board_row_${keyword.categoryResourceId}`}>
          {keyword.keyword}
        </Text>,
        addTrashbin(
          keyword.categoryResourceId,
          onClickDeleteCategoryResourceButton,
        ),
      ])
    })
  }
